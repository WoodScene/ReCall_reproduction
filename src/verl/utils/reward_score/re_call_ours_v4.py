...
# 对v3 的改进：对format和answer同时关注

# 更新format的打分原则，避免模型只学会投机取巧
# Update 1 
#  
...
def extract_and_concatenate_boxed_content(response: str):
    """
    Extracts all the content within \boxed{} tags and concatenates them into a single string.
    
    Args:
        response (str): The full response string containing \boxed{} tags.

    Returns:
        str: The concatenated content from all \boxed{} tags.
    """
    # Step 1: Find all content within \boxed{...} using regex
    boxed_contents = re.findall(r'\\boxed{(.*?)}', response, flags=re.DOTALL)
    
    # Step 2: Concatenate all extracted boxed contents into one string
    concatenated_result = ' '.join(boxed_contents).strip()
...
def validate_template_format(text: str):
    """
    ...
    for response in assistant_responses:
        ...
        if think_count != 0:
            ...
            # 4. check the content of each key_info_save can be parsed as json
            current_pos = 0
    ...
    if final_answer_count != 1: # 只能有一组answer pair
        return 0, f" <answer></answer>出现的次数不是一次！出现的次数是：{final_answer_count} ", response_return
    ...
    # 4. check if the last response contains <answer> <\answer>
    if "<answer>" not in assistant_responses[-1]:
        return 0, "the last response is missing <answer>", response_return
    if "</answer>" not in assistant_responses[-1]:
        return 0, "the last response is missing </answer>", response_return
    
    # # 4. check if the last response contains \\boxed
    # if "\\box" not in assistant_responses[-1]:
    #     return 0, "the last response is missing \\boxed", response_return

    format_score = 0
    reason = ""
    # 这里三种格式的得分应该是递进的，而不是独立得分
    ...
    # 再换一种格式
    # if macro_tool_call_format_flag and key_info_save_format_flag and micro_tool_call_format_flag:
    #     format_score += 0.1

    # 再换一种格式
    if macro_tool_call_format_flag and key_info_save_format_flag:
        format_score += 0.1
...
def compute_score_with_format(tokenizer, solution_str, ground_truth) -> tuple[float, str]:
    ...
    if not solution_str.endswith(tokenizer.eos_token): # 先检查格式是否正确
        print(f'not end with eos token, reward is 0!')
        #print(solution_str)
        #print()
        return 0, f'not end with eos token'

    full_format_score = 0.3
    ...
    if valid_template_score < full_format_score:
        print(f"格式不完全正确，format reward 为 {valid_template_score}！ 格式的满分是 {full_format_score} 分")
        return valid_template_score
    else:
        format_score = valid_template_score

    try:
        answer = remove_micro_tool_calls_and_responses(response)
        # answer = extract_and_concatenate_boxed_content(response)
    ...
    print("格式完全正确，计算答案的F1 score！solution_str is:")
    print(solution_str)
    print()
    print("\n" + "-"*80)
    print(f"final response in box is {answer}")
