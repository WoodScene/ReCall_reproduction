...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        elif 'vllm' in self.config.rollout.name: # 我们用的这里
            if self.config.rollout.name == 'vllm':
                from verl.workers.rollout.vllm_rollout import vLLMRollout, vllm_mode
                rollout_class = vLLMRollout
            elif self.config.rollout.name == 'vllm_with_tool': # recall是这里
                from verl.workers.rollout.vllm_rollout import vLLMRolloutWithTool, vllm_mode
                rollout_class = vLLMRolloutWithTool
            elif self.config.rollout.name == 'vllm_with_micro_tool': # 微宏观检索是这里
                from verl.workers.rollout.vllm_rollout import vLLMRolloutWithMicroTool, vllm_mode
                print("启动微宏观检索！")
                rollout_class = vLLMRolloutWithMicroTool
            else:
                raise NotImplementedError(f'Rollout name {self.config.rollout.name} is not supported')

            # 回放分片管理：每种回放模式有一个对应的 sharding manager，负责数据分片和模型参数的同步。
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        # rollout_sharding_manager负责每一个大 step 结束后把刚刚训练好的 actor model 参数更新到 vllm 或 sglang，这样下一个大 step 的 rollout 采用的就是最新的模型权重（最新的策略）了。
